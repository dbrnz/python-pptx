...
class Evaluator(object):
    ...
    formulae = {
        "*/": lambda x, y, z: value(x)
        * value(y)
        / value(z),  # Multiply Divide Formula
        "+-": lambda x, y, z: value(x)
        + value(y)
        - value(z),  # Add Subtract Formula
        "+/": lambda x, y, z: (value(x) + value(y))
        / value(z),  # Add Divide Formula
        "?:": lambda x, y, z: value(y)
        if value(x) > 0
        else value(z),  # If Else Formula
        "at2": lambda x, y: (
            st_angle(math.atan(value(y) / value(x)))
            if value(x) != 0.0  # ArcTan Formula
            else value("cd4" if value(y) >= 0 else "3cd4")
        ),
        "tan": lambda x, y: value(x)
        * math.tan(radians(y)),  # Tangent Formula
        "cat2": lambda x, y, z: (
            value(x)
            * math.cos(math.atan(value(z) / value(y)))
            if value(y) != 0.0  # Cosine ArcTan Formula
            else 0.0
        ),
        "cos": lambda x, y: value(x)
        * math.cos(radians(y)),  # Cosine Formula
        "sat2": lambda x, y, z: (
            value(x)
            * math.sin(math.atan(value(z) / value(y)))
            if value(y) != 0.0  # Sine ArcTan Formula
            else value(x)
            if value(z) >= 0
            else -value(x)
        ),
        "sin": lambda x, y: value(x)
        * math.sin(radians(y)),  # Sine Formula
        "mod": lambda x, y, z: math.sqrt(
        ...
        "pin": lambda x, y, z: (
            value(x)
            if value(y) < value(x)  # Pin To Formula
            else value(z)
            if value(y) > value(z)
            else value(y)
        ),
